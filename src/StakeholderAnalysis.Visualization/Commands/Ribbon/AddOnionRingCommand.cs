...
namespace StakeholderAnalysis.Visualization.Commands.Ribbon
{
    public class AddOnionRingCommand : ICommand
    {
        ...
        {
            var percentage = 1.0;
            var onionRingsSmallerThanOne = onionDiagram.OnionRings.Where(r => r.Percentage < 1.0)
                .Select(r => r.Percentage).ToArray();
            if (onionRingsSmallerThanOne.Any())
                percentage = (onionRingsSmallerThanOne.Max() + 1.0) / 2.0;
            else if (onionDiagram.OnionRings.Count != onionRingsSmallerThanOne.Length) percentage = 0.5;
